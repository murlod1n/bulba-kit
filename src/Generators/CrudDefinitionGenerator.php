...
class CrudDefinitionGenerator
{
     ...
     */
    public function generate(string $name, array $fields, array $relationships, array $translatableFields = []): void
    {
        ...
        $stub = $this->getStub('crud-definition');

        $fieldsArray = $this->fieldsBuilder->build($fields, $relationships, $translatableFields);
        $rulesArray = $this->rulesBuilder->build($fields, $name, $relationships, $translatableFields);
        $relationsArray = $this->relationsBuilder->build($relationships);

        $mediaCollections = $this->buildMediaCollections($fields);
        $mediaGalleries = $this->buildMediaGalleries($fields);
        ...
        File::put($resourcePath.'/'.$name.'CrudDefinition.php', $content);
    }
    ...
    public function addInverseRelation(
        ...
        $resourceNamespace = config('bulba.resource_namespace', 'App\\Resources');
        $resourcePath = $this->resolveResourcePath($resourceNamespace);
        $path = $resourcePath.'/'.$targetModel.'CrudDefinition.php';
    ...
    /**
     * Build mediaCollections() method content for image and gallery fields.
     *
     * @param  array<int, array<string, mixed>>  $fields  Field definitions
     * @return string Method content
     */
    protected function buildMediaCollections(array $fields): string
    {
        $mediaFields = array_filter($fields, fn ($f) => in_array($f['type'] ?? '', ['image', 'gallery']));

        if (empty($mediaFields)) {
            return '            // no media collections';
        }

        $entries = [];
        foreach ($mediaFields as $field) {
            $collection = $field['modifiers']['collection'] ?? ($field['type'] === 'gallery' ? Str::snake(Str::plural($field['name'])) : $field['name']);
            $entries[] = "            '{$field['name']}' => '{$collection}',";
        }
    ...
    /**
     * Build mediaGalleries() method content for gallery fields.
     *
     * @param  array<int, array<string, mixed>>  $fields  Field definitions
     * @return string Method content
     */
    protected function buildMediaGalleries(array $fields): string
    {
